...
import { GumletService, type VideoUploadOptions } from "./gumlet-service";
import { db } from "@/db";
import {
    athletePrs,
...
export class AthleteService {
    ...
    static async completePRWithVideo(
        ...
        const pr = await this.logPr(boxId, athleteId, movementId, prData.value, prData.unit, {
            reps: prData.reps,
            notes: prData.notes,
            coachNotes: prData.coachNotes,
            achievedAt: prData.achievedAt,
            verifiedByCoach: false, // Will be verified later if needed
            videoData: {
    ...
    /**
     * Compare two PR videos for form analysis
     */
    static async comparePRVideos(
        currentPRId: string,
        previousPRId?: string
        ...
        const currentPR = await db
            .select()
            .from(athletePrs)
            .where(eq(athletePrs.id, currentPRId))
            .limit(1);
        ...
        // Mock form analysis - in a real implementation, this would integrate
        // with AI video analysis services or computer vision APIs
        const analysisResult: FormAnalysisResult = {
            technicalScore: Math.floor(Math.random() * 30) + 70, // 70-100 score
            recommendations: [
            ...
            keyPoints: [
                {
                    timestamp: 1.5,
                    issue: "Hip shift at bottom position",
                    severity: 'medium',
                    suggestion: "Focus on even weight distribution"
                },
                {
                    timestamp: 3.2,
                    issue: "Slight forward lean on ascent",
                    severity: 'low',
                    suggestion: "Drive through heels and maintain chest up"
                }
            ]
        };

        // If comparing with previous PR, add comparison metrics
        if (previousPRId) {
            const previousPR = await db
                .select()
                .from(athletePrs)
                .where(eq(athletePrs.id, previousPRId))
                .limit(1);

            if (previousPR.length) {
                const weightDifference = Number(currentPR[0].value) - Number(previousPR[0].value);
                analysisResult.comparisonMetrics = {
                    previousPR: {
                        improvement: weightDifference,
